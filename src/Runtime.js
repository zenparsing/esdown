...
var runLater = (function() {
  ...
  if (Observer) {
    ...
    var observer$0 = new Observer(function() {
      var fn = queuedFn$0;
      queuedFn$0 = null;
      fn();
    });
...
function polyfill() {
  ...
  function Promise(resolver) { var __this = this; 
    ...
    try {
      resolver(function(x) { resolvePromise(__this, x); }, function(r) { rejectPromise(__this, r); });
    } catch (e) {
  ...
  function coerce(constructor, x) {
    if (!isPromise(x) && Object(x) === x) {
      ...
      try {
        ...
        return makeRejected(constructor, r);
      }
      ...
      if (typeof then$0 === 'function') {
        ...
        try {
          then$0.call(x, deferred$0.resolve, deferred$0.reject);
        } catch (r) {
          deferred$0.reject(r);
        }
  ...
  function enqueueHandlers(value, tasks) {
...
'use strict'; var addProperties = __M(2, 1).addProperties, toLength = __M(2, 1).toLength, toInteger = __M(2, 1).toInteger, sameValue = __M(2, 1).sameValue, assertThis = __M(2, 1).assertThis, isRegExp = __M(2, 1).isRegExp;
...
function polyfill() {
  [symbols, array, mapset, number, object, promise, string].forEach(function(m) {
    m.polyfill(global);
  });
}
